...
#[test]
fn default_run_generates_one_human_html_entrypoint() {
    let temp = create_fixture_repo();
    let repo = temp.path();
    ...
    assert!(
        ...
        "the default pack should not publish a competing static HTML entrypoint"
    );
    assert!(
    ...
    assert!(
        payload["artifacts"]["review_html"].is_null(),
        "compact JSON must not link to an absent static HTML export"
    );
    assert_eq!(
